      ...
      return (
        <div className="min-h-[400px] flex items-center justify-center p-8">
          <div className="text-center max-w-md">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-earth-100 mb-2">
            ...
            {process.env.NODE_ENV === 'development' && this.state.error && (
              <details className="mt-6 text-left">
                <summary className="cursor-pointer text-sm text-gray-500 dark:text-earth-400 hover:text-gray-700 dark:hover:text-earth-300">
                  Error Details (Development)
                </summary>
                <pre className="mt-2 p-4 bg-gray-100 dark:bg-earth-800 rounded-lg text-xs overflow-auto max-h-48 dark:text-earth-200">
  ...
  return (
    <div className="min-h-[400px] flex items-center justify-center p-8">
      <div className="text-center max-w-md">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-earth-100 mb-2">{title}</h2>
        <p className="text-gray-600 dark:text-earth-300 mb-6">{description}</p>
        <div className="space-y-3">
          {resetErrorBoundary && (
            <button
...
export function NotFoundError({
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-8">
      <div className="text-center max-w-md">
        <div className="text-8xl font-serif font-bold text-earth-200 dark:text-earth-800 mb-4">404</div>
        <h1 className="text-2xl font-bold text-gray-800 dark:text-earth-100 mb-2">{title}</h1>
        <p className="text-gray-600 dark:text-earth-300 mb-8">{description}</p>
        <div className="space-y-3">
          <Link
            href="/"
...
export function APIError({
  ...
  return (
    <div className="bg-white dark:bg-earth-900 rounded-xl shadow-sm border border-gray-200 dark:border-earth-700 p-8 text-center">
      ...
      {statusCode && (
        <div className="text-sm text-gray-500 dark:text-earth-400 mb-1">Error {statusCode}</div>
      )}
      <p className="text-gray-700 dark:text-earth-200 mb-4">{displayMessage}</p>
      {onRetry && (
        <button
          onClick={onRetry}
