...
function ToastItem({ toast }: { toast: ToastData }) {
  ...
  const typeConfig: Record<ToastType, { icon: ReactNode; bgColor: string; borderColor: string; iconBg: string }> = {
    success: {
      ...
      bgColor: 'bg-white dark:bg-earth-900',
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-gray-900 dark:text-earth-100 text-sm">{toast.title}</p>
          {toast.message && (
            <p className="text-sm text-gray-600 dark:text-earth-300 mt-0.5">{toast.message}</p>
          )}
          {toast.action && (
            <button
              onClick={() => {
              ...
              className="mt-2 text-sm font-medium text-earth-600 hover:text-gray-800 dark:text-earth-400 dark:hover:text-earth-200 transition"
            >
              {toast.action.label}
            </button>
          )}
        ...
        <button
          ...
          className="flex-shrink-0 text-gray-400 hover:text-gray-600 dark:text-earth-500 dark:hover:text-earth-300 transition"
