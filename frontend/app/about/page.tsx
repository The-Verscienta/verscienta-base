...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) {
  return (
    <div className="bg-white dark:bg-earth-900 rounded-xl p-6 shadow-sm border border-earth-100 dark:border-earth-700 hover:shadow-md hover:border-sage-200 transition-all">
      <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-sage-100 to-earth-100 flex items-center justify-center mb-4">
        {icon}
      </div>
      <h3 className="font-serif font-semibold text-gray-800 dark:text-earth-100 mb-2">{title}</h3>
      <p className="text-gray-600 dark:text-earth-300 text-sm leading-relaxed">{description}</p>
    </div>
  );
}
...
function ValueItem({ number, title, description }: { number: number; title: string; description: string }) {
  return (
    <div className="flex items-start gap-4">
      <div className="w-10 h-10 bg-gradient-to-br from-earth-100 to-sage-100 rounded-full flex items-center justify-center flex-shrink-0 border border-earth-200 dark:border-earth-700">
        <span className="text-earth-700 font-bold">{number}</span>
      </div>
      <div>
        <h3 className="font-serif font-semibold text-gray-800 dark:text-earth-100">{title}</h3>
        <p className="text-gray-600 dark:text-earth-300 text-sm leading-relaxed">
          {description}
        </p>
      </div>
    </div>
    ...
    <PageWrapper>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <Section
          ...
          className="mb-8"
        >
          <p className="text-gray-700 dark:text-earth-200 leading-relaxed mb-4">
        ...
        <div className="relative bg-gradient-to-r from-earth-700 via-earth-800 to-sage-800 rounded-2xl p-8 md:p-12 text-white text-center overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/herbs"
                className="inline-flex items-center justify-center gap-2 bg-white dark:bg-earth-900 text-gray-800 dark:text-earth-100 px-6 py-3 rounded-xl font-semibold hover:bg-cream-50 dark:hover:bg-earth-800 transition-all shadow-lg"
              >
                Explore Herbs
